...
// Helper function to humanize all content fields
async function humanizeContent(content: any): Promise<any> {
  ...
  for (const fieldPath of textFields) {
    ...
    if (current && typeof current[key] === 'string' && current[key].length > 30) {
      textsToHumanize.push(current[key])
      fieldRefs.push({ obj: current, key })
    }
  }

  // Batch humanize main fields
  ...
  if (humanized.why_essential?.points) {
    for (let i = 0; i < humanized.why_essential.points.length; i++) {
      if (typeof humanized.why_essential.points[i] === 'string') {
...
- Return ONLY valid JSON, no markdown, no code blocks, no explanations`
    ...
    // Generate HTML with humanized content
    const htmlContent = generateHTMLPage(humanizedContent)
    ...
    return new NextResponse(zipBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="template2-humanized-${Date.now()}.zip"`,
      },
    })
  } catch (error: any) {
