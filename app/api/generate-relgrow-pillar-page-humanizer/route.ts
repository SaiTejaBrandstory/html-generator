...
function generateHTMLPage(content: any, companyName?: string, ctaLink?: string): string {
  ...
  const templatePath = join(process.cwd(), 'public', 'pillar-page', 'relgrow-pillar-page.html')
  ...
  if (content.page_title) {
    html = html.replace(/<title>Comprehensive Real Estate Management in Bangalore<\/title>/, `<title>${escapeHtml(content.page_title)}</title>`)
  }
  ...
  if (content.header?.title) {
    html = html.replace(/<h1 class="page-title">Your Trusted Partner for Real Estate Life Cycle Management<\/h1>/, `<h1 class="page-title">${escapeHtml(content.header.title)}</h1>`)
  }
  ...
  if (content.banner?.main_heading) {
    html = html.replace(/<h1>Complete Real Estate Management in Bangalore<\/h1>/, `<h1>${escapeHtml(content.banner.main_heading)}</h1>`)
  }
  
  // 4. Banner section - CTA button text
  if (content.banner?.cta_button_text) {
    html = html.replace(/>Request a Consultation\s*</, `>${escapeHtml(content.banner.cta_button_text)} `)
  }
  
  // 5. Banner section - service features (4 features)
  if (content.banner?.service_features && content.banner.service_features.length >= 4) {
    html = html.replace(/<span class="service-feature-text">Total Project Lifecycle<\/span>/, `<span class="service-feature-text">${escapeHtml(content.banner.service_features[0])}</span>`)
    html = html.replace(/<span class="service-feature-text">Timely Execution<\/span>/, `<span class="service-feature-text">${escapeHtml(content.banner.service_features[1])}</span>`)
    html = html.replace(/<span class="service-feature-text">RERA Compliance<\/span>/, `<span class="service-feature-text">${escapeHtml(content.banner.service_features[2])}</span>`)
    html = html.replace(/<span class="service-feature-text">Expert Consultancy<\/span>/, `<span class="service-feature-text">${escapeHtml(content.banner.service_features[3])}</span>`)
  }
  
  // 7. Features section
  if (content.features?.title) {
    html = html.replace(/<h2 class="features-title">Why Choose Our Real Estate Services<\/h2>/, `<h2 class="features-title">${escapeHtml(content.features.title)}</h2>`)
  }
  if (content.features?.intro) {
    html = html.replace(/<p class="features-intro">Relgrow offers a complete suite of real estate life cycle management services, guaranteeing project success throughout the entire process.<\/p>/, `<p class="features-intro">${escapeHtml(content.features.intro)}</p>`)
  }
  ...
  if (content.exclusive_benefits?.title) {
    html = html.replace(/<h2 class="exclusive-benefits-title">Relgrow Exclusive Benefits<\/h2>/, `<h2 class="exclusive-benefits-title">${escapeHtml(content.exclusive_benefits.title)}</h2>`)
  }
  ...
  if (content.types?.title) {
    html = html.replace(/<h2 class="types-title">Tailored Real Estate Services<\/h2>/, `<h2 class="types-title">${escapeHtml(content.types.title)}</h2>`)
  }
  if (content.types?.intro) {
    html = html.replace(/<p class="types-intro">Discover the extensive portfolio of real estate management options available at Relgrow that are tailored to the requirements of individual clients.<\/p>/, `<p class="types-intro">${escapeHtml(content.types.intro)}</p>`)
  }
                ...
                <div class="accordion-item${isActive}">
                    <div class="accordion-header" onclick="toggleAccordion(this)">
                        <div class="accordion-icon">
                            <img src="relgrow-images/service-icon-${imageIndex}.svg" alt="${escapeHtml(item.title)}">
                        </div>
  ...
  if (content.coverages?.title) {
    html = html.replace(/<h2 class="coverages-title">Comprehensive Real Estate Services<\/h2>/, `<h2 class="coverages-title">${escapeHtml(content.coverages.title)}</h2>`)
  }
                        ...
                        <div class="coverage-accordion-item${isActive}">
                            <div class="coverage-accordion-header">
                                <div class="coverage-icon">
                                    <img src="relgrow-images/tick-in-circle.svg" alt="Tick">
                                </div>
    ...
    const notCoveredItemsHTML = content.coverages.not_covered_items.map((item: any, index: number) => {
      ...
      return `
                        <div class="coverage-accordion-item${isActive}">
                            <div class="coverage-accordion-header">
                                <div class="coverage-icon">
                                    <img src="relgrow-images/cross-in-circle.svg" alt="Cross">
                                </div>
                                <h3 class="coverage-accordion-title">${escapeHtml(item.title)}</h3>
                                <div class="coverage-accordion-arrow">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  if (content.why_buy?.title) {
    html = html.replace(/<h2 class="why-buy-title">Why Choose Real Estate Life Cycle Management?<\/h2>/, `<h2 class="why-buy-title">${escapeHtml(content.why_buy.title)}</h2>`)
  }
      ...
      `
                    <div class="feature-item">
                        <div class="feature-icon">
                            <img src="relgrow-images/star-icon.svg" alt="Star icon">
                        </div>
  ...
  if (content.factors?.items && content.factors.items.length > 0) {
    const factorsCardsHTML = content.factors.items.map((item: any, index: number) => {
      // Cycle through icons (factors-icon-1.svg through factors-icon-7.svg) - relgrow template
                        ...
                        <div class="factors-card">
                            <div class="factors-card-icon-wrapper">
                                <div class="factors-card-icon">
                                    <img src="relgrow-images/factors-icon-${iconIndex}.svg" alt="${escapeHtml(item.title)} icon">
                                </div>
                            </div>
  ...
  if (content.things_consider?.title) {
    html = html.replace(/<h2 class="things-consider-title">Things to Consider When Choosing Real Estate Management<\/h2>/, `<h2 class="things-consider-title">${escapeHtml(content.things_consider.title)}</h2>`)
  }
  if (content.things_consider?.subtitle) {
    html = html.replace(/<p class="things-consider-subtitle">Select the right partner and services to ensure successful real estate project outcomes.<\/p>/, `<p class="things-consider-subtitle">${escapeHtml(content.things_consider.subtitle)}</p>`)
  }
                        ...
                        <div class="consider-card">
                            <div class="consider-card-icon-wrapper">
                                <div class="consider-card-icon">
                                    <img src="relgrow-images/things-to-consider.svg" alt="${escapeHtml(item.title)} icon">
                                </div>
                            </div>
      ...
      return `
                    <div class="addon-explore-card${hiddenClass}">
                        <div class="addon-explore-icon">
                            <img src="relgrow-images/additional-icon-${iconIndex}.svg" alt="${escapeHtml(item.name)}">
                        </div>
      ...
      return `
                    <a href="javascript:void(0)" class="explore-more-item${hiddenClass}">
                        ...
                        <div class="explore-more-item-icon">
                            <img src="relgrow-images/explore-more-icon.svg" alt="Arrow icon">
                        </div>
                    </a>`
    }).join('\n                    ')
                ...
                <div class="documents-required-card">
                    <div class="documents-required-card-header">
                        <div class="documents-required-icon-wrapper">
                            <img src="relgrow-images/essential-components.svg" alt="${escapeHtml(item.title)}">
                        </div>
    ...
    const listItemsHTML = content.motor_rules.list_items.map((item: string) => 
      `<li class="motor-rules-list-item">
                    <div class="motor-rules-list-icon">
                        <img src="relgrow-images/green-check-icon.svg" alt="Checkmark">
                    </div>
                    ...
                    <div class="why-buy-online-item-header">
                        <div class="why-buy-online-item-left">
                            <div class="why-buy-online-item-icon">
                                <img src="relgrow-images/why-choose-icon-${iconIndex}.svg" alt="${escapeHtml(item.title)}">
                            </div>
  ...
  if (content.faqs?.heading) {
    html = html.replace(/<h2 class="faq-title">FAQs About Real Estate Life Cycle Management<\/h2>/, `<h2 class="faq-title">${escapeHtml(content.faqs.heading)}</h2>`)
  }
  ...
  if (content.reviews?.title) {
    html = html.replace(/<h2 class="reviews-title">Real Estate Management Reviews<\/h2>/, `<h2 class="reviews-title">${escapeHtml(content.reviews.title)}</h2>`)
  }
...
- Return ONLY valid JSON, no markdown, no code blocks, no explanations`
    ...
    // Add pillar-page relgrow-images directory
    const pillarImagesPath = join(process.cwd(), 'public', 'pillar-page', 'relgrow-images')
    try {
      if (statSync(pillarImagesPath).isDirectory()) {
        addDirectoryToZip(archive, pillarImagesPath, 'relgrow-images')
        console.log('✓ Added pillar-page relgrow-images directory to ZIP')
      }
    } catch (e) {
      console.warn('Pillar-page relgrow-images directory not found, skipping...')
    }
    ...
    return new NextResponse(zipBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="relgrow-pillar-page-generated-${Date.now()}-humanizer.zip"`,
      },
    })
  } catch (error: any) {
